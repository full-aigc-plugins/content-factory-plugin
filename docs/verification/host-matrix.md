...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Codex | `.codex-plugin/plugin.json` | Verified offline | OBSERVED: CLI 0.153.4 | Manifest validated only | NOT_RUN | NOT_RUN | NOT_RUN | NOT_RUN |
...
The separate [source-session evidence](host-source-session.json) records one
read-only ZCode CLI session against the checked-in `content-harness`, one Kimi
session attempt blocked by host quota, and Codex manifest validation. The ZCode
session selected only `format → review`, reported the missing content revision,
made no business-service call, and wrote no file. Because none of these runs
installed the plugin, they do not advance the installed-plugin live matrix.
