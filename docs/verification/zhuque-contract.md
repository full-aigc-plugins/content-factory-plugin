...
当前状态为 `PARTIAL_LIVE_WEBSITE`。2026-09-22 使用官网游客额度完成了三次文本检测，并记录了一次未发出网络请求的空文本反例。发送内容来自仓库自有、可再分发的合成 fixture，不含用户私密材料。其中两次新增检测分别绑定 30 条合成原始基线与相应编辑候选稿，用来观察同一项目语料在官网试用中的返回结构，而不是证明作者身份或“去 AI”效果。随后选择官网内置的人工示例三尝试补充人工对照，点击检测后出现交互验证码；没有绕过验证码，也没有把内置预览结果记录为新的真实响应。
...
- 原始基线与编辑候选稿的返回差异只是一组网站观察，不是因果结论，也不是质量门槛；人工对照尝试没有取得响应。
- 人工示例三只记录正文 SHA-256 `0fb04c9319ca0caf0e3ce5414aff18b7b522be8aeb1c9de6a8b1e14e93bb79d1`，正文不入库；尝试状态为 `NOT_RUN_USER_CHALLENGE`，验证码未绕过，响应与额度变化均未观察到。
...
- 人工稿、原始 AI 稿、编辑稿三类独立 API 记录；当前只有原始基线和编辑稿的网站观察，人工稿网站尝试停在交互验证码；
- API 原始响应字节及其不可变哈希；
