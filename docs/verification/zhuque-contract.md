...
- 用户随后提供的页面截图显示，前一次绑定不一致响应实际停留在官网内置“AI 生成文本示例一”，页面可见比例为人工 `0.2631`、疑似 AI `0.2568`、AI `0.4801`，且当日游客额度已用尽。截图 SHA-256 为 `4c5367437a13c44d643d018af42985ee7a65c76adbad098554eb4980c5e9264e`，尺寸为 `1453×842`。由于原图带有无关浏览器标签上下文，仓库只记录哈希和可见字段，不保存原图；这些字段只解释绑定失败，分类仍被拒收。
