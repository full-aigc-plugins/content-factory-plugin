...
当前状态为 `PARTIAL_LIVE_WEBSITE`。2026-09-22 使用官网游客额度完成了三次文本检测，并记录了一次未发出网络请求的空文本反例。发送内容来自仓库自有、可再分发的合成 fixture，不含用户私密材料。其中两次新增检测分别绑定 30 条合成原始基线与相应编辑候选稿，用来观察同一项目语料在官网试用中的返回结构，而不是证明作者身份或“去 AI”效果。
...
- 原始基线与编辑候选稿的返回差异只是一组网站观察，不是因果结论，也不是质量门槛；未提交人工对照稿。
...
- 人工稿、原始 AI 稿、编辑稿三类独立 API 记录；当前只有原始基线和编辑稿的网站观察，没有人工稿网站观察；
- API 原始响应字节及其不可变哈希；
